...
const isUpdate = computed(() => !!illustrationType?.slug)

const pageTitle = computed(() => {
    return isUpdate.value
        ? `Edit ${illustrationType?.name}`
        : 'Create Illustration Type'
})

const saveForm = useForm({
    name: illustrationType?.name || null,
    brief: illustrationType?.brief || null,
    prompt_instruction: illustrationType?.prompt_instruction || null,
})
...
function handleSave() {
    ...
    if (isUpdate.value) {
        intertiaJsRoute.post(
            route('back-office.illustration-types.update', { slug: illustrationType?.slug }),
            { ...saveForm.data(), _method: 'patch' },
            requestConfig
        )
    } else {
        saveForm.post(route('back-office.illustration-types.save'), requestConfig)
    }
}
                ...
                <div class="bg-white border rounded-xl p-5 shadow-sm space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input v-model="saveForm.name" placeholder="Enter illustration type name"
                ...
                <div class="bg-white border rounded-xl p-5 shadow-sm space-y-4">
                    ...
                    <div>
                        ...
                        <textarea v-model="saveForm.prompt_instruction" rows="8"
                            placeholder="Enter the master instruction for AI image generation for this illustration type. This controls the visual style applied across the entire image."
                            class="w-full border rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
