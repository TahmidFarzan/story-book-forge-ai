...
import { canCreateIllustrationType, canUpdateIllustrationType, canDeleteIllustrationType } from '@/composables/useUserPermissions'
...
const { illustrationTypes } = defineProps({
    illustrationTypes: Object,
})

const paginationOnly = computed(() => {
    if (!illustrationTypes) return {}
    const { data, ...rest } = illustrationTypes
    return rest
})
...
const applyFilter = () => {
    ...
    intertiaJsRoute.get(route('back-office.illustration-types.index'), cleanParams, {
        replace: true,
        preserveScroll: true,
        preserveState: true,
...
const confirmDelete = (illustrationType) => {
    deletingRow.value = illustrationType
...
const canCreate = () => canCreateIllustrationType(authUser?.value)
...
const handleDelete = (illustrationType) => {
    if (!illustrationType || deleteProcessing.value) return
...
onMounted(async () => {
    ...
    window.dispatchEvent(
        new CustomEvent('set-breadcrumb', {
            detail: [
                { text: 'Illustration Types', active: true },
            ],
        })
    )
})
...
<template>
    ...
    <div class="w-full space-y-6">
        ...
        <div class="flex justify-between items-center">
            ...
            <a v-if="canCreate()" :href="route('back-office.illustration-types.create')"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y">
                        <tr v-for="(item, index) in illustrationTypes?.data" :key="item.id" class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <a :href="route('back-office.illustration-types.details', { slug: item.slug })"
                                        class="p-2 rounded-md text-blue-600 hover:bg-blue-50 border"
                                        title="View Details">
                                        <FontAwesomeIcon icon="info" />
                                    </a>

                                    <a v-if="canUpdate(item)" :href="route('back-office.illustration-types.edit', { slug: item.slug })"
                        ...
                        <tr v-if="!illustrationTypes?.data?.length">
                            <td colspan="5" class="px-4 py-8 text-center text-gray-500">
                                No illustration types found
                            </td>
                        </tr>
                    </tbody>
